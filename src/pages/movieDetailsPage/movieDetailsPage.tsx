...
export default function MovieDetailsPage() {
  ...
  return (
    <>
      <div
        className="min-h-dvh text-white relative"
        style={{
      ...
      >
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-8 py-4 sm:py-6 pb-12 sm:pb-16">
          <div>
            <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start">
              <div className="flex-shrink-0 w-full md:w-auto flex justify-center md:justify-start">
                <img
                  ...
                  alt={movie.title}
                  className="rounded-2xl shadow-2xl w-full max-w-[260px] sm:max-w-[300px] md:w-80 ring-1 ring-white/20"
                />
              </div>

              <div className="flex-1">
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-semibold pb-4 sm:pb-6 bg-gradient-to-r from-white via-white/90 to-white/70 bg-clip-text text-transparent tracking-wide leading-tight">
                  {movie.title}
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-5 text-xs sm:text-sm">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 mb-6">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/10 ring-1 ring-white/15 flex items-center justify-center hover:bg-white/20 transition backdrop-blur-sm"
                    >
                      <List size={18} />
                    </button>
                    <button
                ...
                {movie.tagline && (
                  <p className="text-white/80 italic text-base sm:text-lg mb-4">
                    “{movie.tagline}”
                  </p>
                )}
          ...
          <div className="mt-8">
            <div className="flex justify-between flex-wrap items-center mb-4">
              ...
              <p className="text-[11px] sm:text-xs uppercase tracking-[0.18em] text-white/50">
                Swipe to explore the cast
              </p>
            </div>
            <div
              className="relative flex gap-4 overflow-x-auto pb-4 pl-2 pt-3"
              style={{
                scrollbarWidth: "thin",
            ...
            >
              {movie.credits?.cast?.slice(0, 10).map((actor) => (
                <Link
                  key={actor.id}
                  to={`/actor/${actor.id}`}
                  className="min-w-[150px] sm:min-w-[155px] max-w-[155px] bg-white/10 ring-1 ring-white/20 backdrop-blur-md overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:bg-white/15 rounded-xl cursor-pointer hover:scale-105"
                >
                  <img
                    src={
                      actor.profile_path
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-white/70 text-xs mt-1 line-clamp-2">{actor.character}</p>
                  </div>
                </Link>
              ))}
            </div>
